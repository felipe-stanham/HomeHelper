...
| P-0006  | Secret Manager  | [ ]         | Per-env master `secrets.env` (operator-edited, mode 600) → SecretManager filters per-app to declared `requires_secrets` → systemd `EnvironmentFile=-` (Linux) / Popen `env=` (Darwin). Refuse-to-start when missing; `GET /api/secrets` listing (no values); zero secret values in any log. |
